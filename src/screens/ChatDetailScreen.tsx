...
import {View, FlatList, StyleSheet, Text, ImageBackground, KeyboardAvoidingView, Platform} from 'react-native';
...
import { getConversationById, markAllMessagesRead, sendMessage } from '../api/messageApi';
...
const [friendUser, setFriendUser] = useState<any>(null);
    const loadMessages = async () => {
        try {
            ...
            const user1Avatar = user1.avatarUrl ? { uri: user1.avatarUrl } : defaultAvatar;
            const user2Avatar = user2.avatarUrl ? { uri: user2.avatarUrl } : defaultAvatar;
 ...
 // 👉 reload friend info (onlineStatus) khi user online/offline
    ...
    const prevDate = index > 0 ? new Date(messages[index - 1].sentAt).toDateString() : null;
    ...
    return (
        <>
            ...
            />
                ...
                onLayout={() => {
                    if (didMount.current) flatListRef.current?.scrollToEnd({ animated: false });
                }}
                onContentSizeChange={() => {
                    if (didMount.current) flatListRef.current?.scrollToEnd({ animated: false });
                }}
