...
import GameResultCard from '../components/common/MatchRankScreen/MatchResultCard2';
...
const HostMatchScreen = ({navigation,route}) => {
...
useEffect(() => {
  ...
  const setUp = async () => {
    try {
      ...
      const userId = route.params?.user; // 👈 Lấy từ route
      const opponentId = route.params?.opponent; // 👈 Lấy từ route

      console.log("🧾 Param userId:", userId);
      console.log("🧾 Param opponentId:", opponentId);
...
const user = accountLogin;
      ...
      const opponent = await FindMatch(userId, opponentId); // ✅ truyền thêm opponentId
      
      console.log("✅ Ghép cặp với:", opponent);

      const matchInfo = createMatchData(user, opponent); // ⚠️ truyền object user nếu accountLogin chưa đúng
      ...
      const opponentIdCalc =
        matchInfo.playerWhite.userId === userId
          ? matchInfo.playerBlack.userId
          : matchInfo.playerWhite.userId;

      setOpponentId(opponentIdCalc);
      ...
      currentIntervalId.current = setInterval(() => {
        setTimeBlack((prev) => decreaseTime(prev));
      }, 1000);

      console.log("⏳ Chờ 1s trước khi bắt đầu trận...");
      setTimeout(() => {
        console.log("🎯 Trận đấu bắt đầu!");
        // có thể gọi hiệu ứng hoặc set ready ở đây nếu muốn
      }, 1000);
    } catch (err) {
