...
const ForgotPasswordScreen = ({navigation}: Props) => {
  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      Alert.alert(
        t.success,
        t.forgot_password_email_sent || 'Check your email for the code!',
      );
      setStep(2);
    } catch (error) {
      Alert.alert(
  ...
  const handleResetPassword = async () => {
    if (!code || !newPassword || !confirmPassword)
      return Alert.alert(t.notice, t.notice_fill_all);
    if (newPassword !== confirmPassword)
      return Alert.alert(t.notice, t.notice_password_not_match);
    try {
      await confirmChangePassword(email, code, newPassword);
      Alert.alert(
        t.success,
        t.forgot_password_success || 'Password changed successfully',
      );
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      Alert.alert(
        t.success,
        t.forgot_password_email_sent || 'Code resent. Check your email!',
      );
    } catch (error) {
      Alert.alert(
        t.noti_danger,
        ...
        <View style={styles.body}>
          <View style={{marginTop: 30}}>
            <Text style={styles.title}>
              {t.forgot_password_heading || 'Reset your password'}
            </Text>
          </View>
          ...
          {step === 2 && (
            <>
              <Text style={styles.label}>
                {t.enter_code_label || 'Enter code'}
              </Text>
              <TextInput
                ref={codeInputRef}
                style={styles.input}
                placeholder={
                  t.enter_code_placeholder ||
              ...
              <View style={styles.bottomButtonsContainer}>
                <TouchableOpacity
                  ...
                  <LinearGradient
                    ...
                    style={styles.button}>
                    <Text style={styles.buttonText}>
                      {t.resend_code_button || 'Resend Code'}
                    </Text>
                  </LinearGradient>
                </TouchableOpacity>
